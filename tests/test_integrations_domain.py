...
def test_import_creates_draft_with_imported_from_link(session, gitlab_connection, repo, monkeypatch):
    ...
    # Prompt stays self-sufficient: body quoted, framed as data, never a bare
    # instruction (the agent runs on the prompt alone, without the description).
    assert "> It breaks on suspend." in ticket.prompt
    assert "quoted as reference data" in ticket.prompt
    # Human-facing description carries a readable summary (review surfaces prefer
    # it over the prompt); it is prose, not the quoted-as-data block.
    assert ticket.description is not None
    assert "It breaks on suspend." in ticket.description
    assert "> It breaks on suspend." not in ticket.description
    assert "#12" in ticket.description
