...
def test_agent_token_transition_is_token_attributed_and_never_acks(session, profile):
    """Integration seam B (token-perms x ack-flow): a transition driven by an
    ``ndk_`` agent token records ``actor_kind='token'`` and does NOT auto-ack —
    an agent can never mark its own work seen. A run token maps to ``run``."""
    from nightdesk.domain import api_tokens as at
    from nightdesk.domain.events import actor_from_principal

    agent = at.mint_api_token(session, name="pm", scopes=["tickets.transition"])
    session.commit()
    agent_actor = actor_from_principal(at.resolve_token(session, agent.cleartext))
    assert agent_actor.kind == "token"

    t = _draft(session, profile)
    _to_review(session, t.id, actor=agent_actor)
