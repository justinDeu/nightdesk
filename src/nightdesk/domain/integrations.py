...
def emit_external_link_state_changed(
    ...
    The ack-flow branch owns the real ticket_events bus; until it merges this is
    a no-op beyond a log line. The payload shape is fixed:
    ``{ticket_id, external_link_id, kind, role, old_state, new_state,
    occurred_at}``. Do not grow behavior here — lifecycle policy is explicitly
