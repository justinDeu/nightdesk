...
def _setup_logging() -> None:
    ...
    Routes log output to the per-run log file when NIGHTDESK_RUN_ID is set
    (production bwrap runs). Falls back to basic stderr logging for ad-hoc
    invocations (tests, manual CLI).
    """
    run_id = os.environ.get("NIGHTDESK_RUN_ID")
    if run_id:
        try:
            from nightdesk.logging_setup import per_run_log_handler
            handler = per_run_log_handler(run_id)
