...
**opencode rendering (2026-07-18): `openai_codex` renders onto opencode's native `"openai"` provider id, not `nd_<endpoint_id>`.** Every other protocol renders as a standalone `nd_<endpoint_id>` provider block (see `backends/opencode_config.py`, "one provider block per endpoint") because its credential is a plain inline `options.apiKey` a generic `@ai-sdk/*` package accepts under any block id. `openai_codex` is different: opencode's Codex OAuth handling — token refresh, the Responses-shaped chatgpt-backend routing, the `originator`/`ChatGPT-Account-Id` headers — is a plugin *bundled with the opencode binary itself*, hard-wired to the literal provider id `"openai"` (verified against opencode 1.16.2 source, `packages/opencode/src/plugin/openai/codex.ts`: both the fetch/auth loader and the Responses-routing hook gate on `model.providerID === "openai"`). An `oauth_file` auth entry or provider block rendered under `nd_<eid>` is never consulted by that plugin — a run against it fails with an opaque `AI_LoadAPIKeyError` even though the endpoint's credential is present and valid, because a generic `@ai-sdk/openai` block only knows `options.apiKey`, and nightdesk correctly never puts a raw OAuth access token there. So `resolve_provider_ids` (in `opencode_config.py`) maps an `openai_codex` endpoint's block, auth entry, and every model reference in that run's config/prompt bodies onto `"openai"` instead of its own namespace; the provider block itself emits only a model menu (`provider.openai.models`, which opencode's config-merge layer folds INTO the bundled provider rather than replacing it) plus a display name — never `npm`/`options`, which would tear out the plugin's own aisdk wiring. This means at most one `openai_codex` endpoint per opencode run can actually work: two would both want the `"openai"` slot, so the one backing the `primary` assignment wins and the other is dropped (logged, not crashed) — a known limitation, not a supported multi-account Codex setup. `OPENCODE_AUTH_CONTENT` also fully REPLACES opencode's on-disk auth store for the process rather than merging with it (same source, `auth/index.ts`), which is why nightdesk must render an auth entry for every credentialed endpoint the run touches, not just the ones it's changing.
