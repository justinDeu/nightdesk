...
NOTE (re-chain at final integration): ``down_revision`` points at
``0026_session_kind`` and this revision claims ``0028``, leaving a deliberate
gap. Three sibling feature branches (token-perms, gitlab-integration,
resident-agents) each add a migration; the final integration linearizes them to
``0027``..``0029`` and repoints this ``down_revision`` accordingly.
"""
